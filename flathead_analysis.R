...
# Get the data for this species
  # Length, weight, age data
    gdat <- flathead_growth
  # Catch curve data
    cdat <- data.frame(
      flathead_mort[ ,1],
      stack(flathead_mort[ ,2:ncol(flathead_mort)])
    )  
...
# Uncomment to reproduce analysis.
  # flathead <- stan(file = 'models/population_analysis_wcc.stan',
  #             data = vb_data,
  #             chains = 3,
  #             iter = 10000,
  #             warmup = 9000,
  ...
  # save(flathead, file="results/flathead_posts.rda")

# Load results from previously fit model
  load("results/flathead_posts.rda")  
  
# Print model summary
#  print(flathead, digits=3)
  
# Extract posteriors
  pars <- rstan::extract(flathead)
  list2env(pars, envir = .GlobalEnv)

# . Derived quantities ----
# Age at length from posteriors
  t330 = -1/K * log(1 - 330.2/Linf) + t0 # Age for min Length
  t736 = -1/K * log(1 - 736.6/Linf) + t0 # Age at 29"
...
# Write the table out to a file  
  write.table(out_data, file="results/flathead_summary_stan.csv",
...
# . sim function ----
  ...
  # Ages corresponding to length limits  
    Tc <- mean(t330)
    Tmax <- min(c(max(age), min(t736, na.rm = TRUE)))
...
# Write ypr summary for plotting
  save(ypr, file='results/flathead_ypr.rda')
  
# Write F_msy estimates to a file
  flathead_F <- do.call(rbind, mget(ls()[grep(pattern='_fmsy', x=ls())]))
  row.names(flathead_F) <- gsub(
    pattern="_fmsy", x=row.names(flathead_F), replacement = ""
    )
  colnames(flathead_F) <- c('mean', 'lwr', 'upr')
  write.csv(flathead_F, file='results/flathead_fmsy.csv', quote=FALSE)
  
# Write MSY estimates to a file
  flathead_msy <- do.call(rbind, mget(ls()[grep(pattern='_msy', x=ls())]))
  row.names(flathead_msy) <- gsub(
    pattern="_msy", x=row.names(flathead_msy), replacement = ""
    )
  colnames(flathead_msy) <- c('mean', 'lwr', 'upr')
  write.csv(flathead_msy, file='results/flathead_msy.csv', quote=FALSE)
