...
# Get the data for this species
  ...
  # Catch curve data
    cdat <- data.frame(
      blue_mort[ ,1],
      stack(blue_mort[ ,2:ncol(blue_mort)])
    )  
...
# Uncomment to reproduce analysis.
  blue <- stan(file = 'models/population_analysis_wcc.stan',
              data = vb_data,
              chains = 3,
              iter = 10000,
              warmup = 9000,
  ...
  # save(blue, file="results/blue_posts.rda")
  
# Load results from previously fit model
  load("results/blue_posts.rda")  
  
# Print model summary
  print(blue, digits=3)
  
# Extract posteriors
  pars <- rstan::extract(blue)
...
# Write the table out to a file  
  write.table(out_data, file="results/blue_summary_stan.csv",
...
# . sim function ----
  ...
  # Ages corresponding to length limits  
    Tc <- mean(t330)
    Tmax <- min(c(max(age), mean(t889, na.rm = TRUE)))
...
# Write ypr summary for plotting
  save(ypr, file='results/blue_ypr.rda')
  
# Write F_msy estimates to a file
  blue_F <- do.call(rbind, mget(ls()[grep(pattern='_fmsy', x=ls())]))
  row.names(blue_F) <- gsub(
    pattern="_fmsy", x=row.names(blue_F), replacement = ""
    )
  colnames(blue_F) <- c('mean', 'lwr', 'upr')
  write.csv(blue_F, file='results/blue_fmsy.csv', quote=FALSE)
  
# Write MSY estimates to a file
  blue_msy <- do.call(rbind, mget(ls()[grep(pattern='_msy', x=ls())]))
  row.names(blue_msy) <- gsub(
    pattern="_msy", x=row.names(blue_msy), replacement = ""
    )
  colnames(blue_msy) <- c('mean', 'lwr', 'upr')
  write.csv(blue_msy, file='results/blue_msy.csv', quote=FALSE)
